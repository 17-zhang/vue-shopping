<template>
  <div class="cart">
    ...
    <main class="cart-body" v-if="cartGoods.length > 0">
      <div class="wrap">
        <ul class="goods-wrap">
          <li v-for="item in cartGoods" :key="item.id" class="cartGoodsItem">
            <cart-one-item
              :itemId="item.id"
              :imgUrl="item.img"
              :title="item.title"
              ...
              :price="item.price"
              :count="item.count"></cart-one-item>
          </li>
        </ul>
        <!--价格详情-->
            ...
            <div>红包：
              <el-select v-model="redPacket" :placeholder="redPacketPlaceholder">
                <el-option
                  ...
                  :label="item.label"
                  :value="item.money"
                  :disabled="isRedPacketDisabled(item.limit)"></el-option>
              </el-select>
    ...
    <!--购物车结算-->
    <footer class="cart-foot" v-show="cartGoods.length > 0">
      <div class="toPay">
        <el-button type="danger" class="text-right" @click="toOrderForm">去结算
